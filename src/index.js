...
export default class VideoLooper extends React.Component {
  ...
  componentDidMount() {
    ...
    this.videoClone.addEventListener(
      "loadeddata",
      this.onLoadedVideoClone.bind(this)
    );
  ...
  componentDidUpdate(prevProps) {
    ...
    if (
      ...
      this.props.start < this.video.duration
    ) {
  ...
  checkLoopEnd(currentVideo, nextVideo) {
    if (
      currentVideo.currentTime >= this.props.end &&
      (!this.props.loopCount || this.state.currentLoop < this.props.loopCount)
    ) {
      nextVideo.play();
...
const Video = styled.video`
  position: ${(props) =>
    props.isDebugMode && props.isSplitView ? "relative" : "absolute"};
  object-fit: ${(props) => props.objectFit};
  object-position: ${(props) => props.objectPosition};
  width: ${(props) =>
    props.isDebugMode && props.isSplitView ? "50%" : "100%"};
  height: 100%;
  left: 0;
  opacity: ${(props) =>
    props.isVisible || (props.isDebugMode && props.isSplitView) ? 1 : 0};
  &.videoClone {
    filter: ${(props) =>
      props.isDebugMode && props.isSplitView ? "grayscale(100%)" : "none"};
  }
`;
...
const PlayButton = styled.div`
  ...
  top: 50%;
  left: ${(props) =>
    props.isDebugMode && props.isSplitView
      ? props.isVideoCloneActive
        ? "75%"
        : "25%"
      : "50%"};
